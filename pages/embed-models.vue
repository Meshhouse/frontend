...
export default class EmbedModelsPage extends Vue {
  ...
  async asyncData ({ app, error }: { app: NuxtApp, error: any }): Promise<{ pageContent: StaticPage } | undefined> {
    try {
      const pageContent: StaticPage = (await app.$api({
        method: 'GET',
        url: 'pages/static/embed-models',
        headers: app.$generateAuthHeader('pages/static/embed-models', 'GET')
      })).data
